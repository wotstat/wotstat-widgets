package wotstat.widgets {
  ...
  public class MainViewOld extends AbstractView {
    ...
    private var activeWidgets:Vector.<DraggableWidgetOld> = new Vector.<DraggableWidgetOld>();
    private var activeWidgetsByWid:Object = new Object();

    private var isInBattle:Boolean = false;

    public function MainViewOld() {
      super();
    }
    ...
    override protected function onDispose():void {
      ...
      for each (var widget:DraggableWidgetOld in activeWidgets) {
        widget.dispose();
      }
      super.onDispose();
    }
    ...
    private function setWidgetLayer(widget:DraggableWidgetOld, layer:String):void {
    ...
    public function as_createWidget(wid:int, url:String, width:int, height:int, x:int, y:int,
      ...
      var widget:DraggableWidgetOld = new DraggableWidgetOld(wid, width, height, x, y, isHidden, isLocked, isControlsAlwaysHidden, isInBattle, positionMode, layer);
      widget.addEventListener(DraggableWidgetOld.REQUEST_RESIZE, onWidgetRequestResize);
      widget.addEventListener(DraggableWidgetOld.REQUEST_RESOLUTION, onWidgetRequestResolution);
      widget.addEventListener(DraggableWidgetOld.MOVE_WIDGET, onWidgetMove);
      widget.addEventListener(DraggableWidgetOld.LOCK_WIDGET, onWidgetLockUnlock);
      widget.addEventListener(DraggableWidgetOld.UNLOCK_WIDGET, onWidgetLockUnlock);
      widget.addEventListener(DraggableWidgetOld.SHOW_WIDGET, onWidgetHideShow);
      widget.addEventListener(DraggableWidgetOld.HIDE_WIDGET, onWidgetHideShow);
    ...
    public function as_onFrame(wid:int, width:int, height:int, data:Array, shift:int):void {
      ...
      var widget:DraggableWidgetOld = activeWidgetsByWid[wid];
      if (widget == null)
        return;
    ...
    public function as_closeWidget(wid:int):void {
      ...
      widget.dispose();
      widget.removeEventListener(DraggableWidgetOld.REQUEST_RESIZE, onWidgetRequestResize);
      widget.removeEventListener(DraggableWidgetOld.REQUEST_RESOLUTION, onWidgetRequestResolution);
      widget.removeEventListener(DraggableWidgetOld.MOVE_WIDGET, onWidgetMove);
      widget.removeEventListener(DraggableWidgetOld.LOCK_WIDGET, onWidgetLockUnlock);
      widget.removeEventListener(DraggableWidgetOld.UNLOCK_WIDGET, onWidgetLockUnlock);
      widget.removeEventListener(DraggableWidgetOld.SHOW_WIDGET, onWidgetHideShow);
      widget.removeEventListener(DraggableWidgetOld.HIDE_WIDGET, onWidgetHideShow);
    }
    ...
    private function onWidgetRequestResize(event:ResizeEvent):void {
      if (this.py_requestResize != null) {
        var widget:DraggableWidgetOld = event.target as DraggableWidgetOld;
